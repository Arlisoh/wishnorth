...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await db.from("wish_lists").insert({
      subject_name: subjectName,
      occasion,
      is_managed: Boolean(body.isManaged),
      owner_key_hash: hashKey(ownerKey),
      owner_user_id: user?.id || null,
      share_token: shareToken,
    }).select("id, share_token").single();
    if (error) throw error;
    return NextResponse.json({ id: data.id, ownerKey, shareToken: data.share_token, savedToAccount: Boolean(user) });
  } catch (e) {
    console.error(e);
