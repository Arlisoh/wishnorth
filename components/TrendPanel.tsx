...
function ProductCard({ product }: { product: Product }) {
  const movementClass = product.wishShareChange > 0 ? "positive" : product.wishShareChange < 0 ? "negative" : "";
  return <article className="index-product-card"><div className="index-product-image">{product.imageUrl ? <img src={productImageUrl(product.imageUrl)} alt={product.title} loading="lazy" /> : <div className="index-thumb-placeholder">🎁</div>}<span className="index-rank">#{product.rank}</span></div><div className="index-product-copy"><div className="index-product-context"><span className="index-category">{product.category || "Other"}</span><span className="index-retailer">{product.retailer || "Retailer not listed"}</span></div><h4>{product.title}</h4><div className="index-score-row"><span><b>{product.wishVelocity}</b><small>Velocity</small></span><span><b>{product.wishShare}%</b><small>Wish share</small></span><span className={movementClass}><b>{product.wishShareChange > 0 ? "+" : ""}{product.wishShareChange}</b><small>Movement</small></span></div><div className="index-product-footer"><div className="index-product-meta"><strong>{product.count} {product.count === 1 ? "wish" : "wishes"}</strong>{product.priceCents != null ? <span>{money(product.priceCents, product.currency || "USD")}</span> : null}<span>{product.uncoveredCount} open</span></div>{product.url ? <a href={product.url} target="_blank" rel="noopener noreferrer" aria-label={`View ${product.title}`}>View product <span aria-hidden="true">↗</span></a> : null}</div></div></article>;
}
